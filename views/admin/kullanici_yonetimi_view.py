...
class SifreDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        info.setStyleSheet("color: #2c3e50; font-size: 13px; padding: 8px;")
...
class KullaniciYonetimiView(QWidget):
    ...
    def setup_ui(self):
        ...
        # We'll manage widths programmatically for 0,1,3,5; keep 2 and 4 content-based
        header.setSectionResizeMode(0, QHeaderView.Fixed)           # Ad Soyad (managed)
        header.setSectionResizeMode(1, QHeaderView.Fixed)           # E-posta (managed)
        header.setSectionResizeMode(2, QHeaderView.ResizeToContents) # Rol
        header.setSectionResizeMode(3, QHeaderView.Fixed)           # Bölüm (managed)
        header.setSectionResizeMode(4, QHeaderView.ResizeToContents) # Son Giriş
        header.setSectionResizeMode(5, QHeaderView.Fixed)           # İşlemler (managed)
    ...
    def load_users(self):
        ...
        try:
            ...
            for user in users:
                ...
                self.table.setItem(row, 3, QTableWidgetItem(bolum))
                ...
                self.table.setItem(row, 4, QTableWidgetItem(son_giris))
                
                # Actions
                action_widget = QWidget()
                action_widget.setMinimumWidth(300) # Ensure enough space for buttons
                ...
                password_btn.setMinimumWidth(90)
                password_btn.setStyleSheet("background-color: #f39c12; color: white; border: none; border-radius: 5px; font-weight: 600;")
    ...
    def adjust_columns(self):
        """Proportionally distribute remaining width to name/email/department and keep actions visible."""
        ...
        # Measure content-based columns
        role_w = max(110, self.table.sizeHintForColumn(2))
        login_w = max(150, self.table.sizeHintForColumn(4))

        # Estimate actions width from widgets
        actions_w = 0
        for row in range(self.table.rowCount()):
            cw = self.table.cellWidget(row, 5)
            if cw:
                actions_w = max(actions_w, cw.sizeHint().width())
        if actions_w == 0:
            actions_w = 260
        actions_w = max(240, min(actions_w, 360))

        reserved = role_w + login_w + actions_w
        remaining = max(300, viewport_w - reserved - scrollbar_w)

        # Distribute remaining: Name 40%, Email 35%, Dept 25%
        name_w = max(160, int(remaining * 0.40))
        email_w = max(200, int(remaining * 0.35))
        dept_w = max(140, remaining - name_w - email_w)

        # Apply widths
        self.table.setColumnWidth(0, name_w)
        self.table.setColumnWidth(1, email_w)
        self.table.setColumnWidth(3, dept_w)
